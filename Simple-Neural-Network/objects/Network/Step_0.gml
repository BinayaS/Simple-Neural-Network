if(update) {
	...
	for(var i = 0; i < InputLayerSize; i++) {
		var _neuron = ds_list_find_value(InputLayer, i);
		_neuron.showSelf = showNetwork;
	}
	for(var i = 0; i < Hidden1LayerSize; i++) {
		var _neuron = ds_list_find_value(Hidden1Layer, i);
		_neuron.showSelf = showNetwork;
	}
	for(var i = 0; i < Hidden2LayerSize; i++) {
		var _neuron = ds_list_find_value(Hidden2Layer, i);
		_neuron.showSelf = showNetwork;
	}
	for(var i = 0; i < Hidden3LayerSize; i++) {
		var _neuron = ds_list_find_value(Hidden3Layer, i);
		_neuron.showSelf = showNetwork;
	}
	for(var i = 0; i < OutputLayerSize; i++) {
		var _neuron = ds_list_find_value(OutputLayer, i);
		_neuron.showSelf = showNetwork;
	}
